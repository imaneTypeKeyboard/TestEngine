...
void ClientNetwork::ClientNetwork()
{
...
int ClientNetwork::Connect(const char *host, const char *serv, int timeout)
{
	...
	error = 0;
	if ( (n = connect(fd_, saptr, salen)) < 0)
	{
	...
	n = select(fd_+1, &rset, &wset, NULL, nsec ? &tval : NULL);
	if ( n <= 0) {
		close(fd_);		/* timeout */
		if (n < 0)
			strerror_r(errno, buf, 1024);
...
int ClientNetwork::SendSync(char *pbuffer, int len)
{
	int n;
	int already = 0;
	if (len < 0)
